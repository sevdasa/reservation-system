...
function submit() {
  form.post(route('bookable.time-slots.store'))
}
</script>
    ...
    <form @submit.prevent="submit" class="space-y-6">
      <div class="transition-opacity duration-300">
        ...
        <DatePickerRoot id="date-field" :v-model="form.date" :locale="locale"
          class="mt-4 rounded-xl bg-gray-50 p-5 shadow-inner" fixed-weeks
          ...
          <DatePickerField v-slot="{ segments }"
            ...
            <div class="flex space-x-1 rtl:space-x-reverse">
              <template v-for="item in segments" :key="item.part">
                <DatePickerInput v-if="item.part === 'literal'" :part="item.part" class="select-text">
                  {{ item.value }}
                </DatePickerInput>
